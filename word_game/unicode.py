...
FINAL = 0x100
CHAR_LISTS = {
    INITIAL: list(map(unichr, [
        0x3131, 0x3132, 0x3134, 0x3137, 0x3138, 0x3139,
        0x3141, 0x3142, 0x3143, 0x3145, 0x3146, 0x3147,
        0x3148, 0x3149, 0x314a, 0x314b, 0x314c, 0x314d,
        0x314e
    ])),
    MEDIAL: list(map(unichr, [
        0x314f, 0x3150, 0x3151, 0x3152, 0x3153, 0x3154,
        0x3155, 0x3156, 0x3157, 0x3158, 0x3159, 0x315a,
        0x315b, 0x315c, 0x315d, 0x315e, 0x315f, 0x3160,
        0x3161, 0x3162, 0x3163
    ])),
    FINAL: list(map(unichr, [
...
def check_hangul(c, jamo_only=False):
    if not ((jamo_only or is_hangul_compat_jamo(c)) or is_supported_hangul(c)):
        raise ValueError("'{c}' is not a supported hangul character. "
                         "'Hangul Syllables' (0xac00 ~ 0xd7a3) and "
                         "'Hangul Compatibility Jamos' (0x3130 ~ 0x318f) are "
                         "supported at the moment.")


def get_jamo_type(c):
    check_hangul(c)
    assert is_hangul_compat_jamo(c), "not a jamo: {ord(c):x}"
...
def split_syllables(s, ignore_err=True, pad=None):
    ...
    def try_split(c):
        try:
            ...
            if ignore_err:
                return (c,)
            raise ValueError("encountered an unsupported character: "
                             "{c} (0x{ord(c):x})")
...
def join_jamos(s, ignore_err=True):
    ...
    def flush(n=0):
        ...
        if len(new_queue) == 1:
            if not ignore_err:
                raise ValueError("invalid jamo character: {new_queue[0]}")
            result = new_queue[0]
            ...
            try:
                ...
                if not ignore_err:
                    raise ValueError("invalid jamo characters: {new_queue}")
