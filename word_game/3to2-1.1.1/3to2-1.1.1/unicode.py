...
__all__ = [u"split_syllable_char", u"split_syllables",
           u"join_jamos", u"join_jamos_char",
           u"CHAR_INITIALS", u"CHAR_MEDIALS", u"CHAR_FINALS"]
...
FINAL = 0x100
CHAR_LISTS = {
    INITIAL: list(imap(unichr, [
        0x3131, 0x3132, 0x3134, 0x3137, 0x3138, 0x3139,
        0x3141, 0x3142, 0x3143, 0x3145, 0x3146, 0x3147,
        0x3148, 0x3149, 0x314a, 0x314b, 0x314c, 0x314d,
        0x314e
    ])),
    MEDIAL: list(imap(unichr, [
        0x314f, 0x3150, 0x3151, 0x3152, 0x3153, 0x3154,
        0x3155, 0x3156, 0x3157, 0x3158, 0x3159, 0x315a,
        0x315b, 0x315c, 0x315d, 0x315e, 0x315f, 0x3160,
        0x3161, 0x3162, 0x3163
    ])),
    FINAL: list(imap(unichr, [
...
CHAR_SETS = dict((k, set(v)) for k, v in CHAR_LISTS.items())
CHARSET = set(itertools.chain(*CHAR_SETS.values()))
CHAR_INDICES = dict((k, dict((c, i) for i, c in enumerate(v)))
...
def split_syllable_char(c):
    ...
    check_hangul(c)
    if len(c) != 1:
        raise ValueError(u"Input string must have exactly one character.")
    ...
    return tuple(CHAR_LISTS[pos][idx] if idx is not None else None
                 for pos, idx in
                 izip([INITIAL, MEDIAL, FINAL], [init, med, final]))
...
def split_syllables(s, ignore_err=True, pad=None):
    ...
    s = imap(try_split, s)
    if pad is not None:
        tuples = imap(lambda x: tuple(pad if y is None else y for y in x), s)
    else:
        tuples = imap(lambda x: ifilter(None, x), s)
    return u"".join(itertools.chain(*tuples))
...
def join_jamos_char(init, med, final=None):
    ...
    for c in ifilter(None, chars):
        check_hangul(c, jamo_only=True)

    idx = tuple(CHAR_INDICES[pos][c] if c is not None else c
                for pos, c in izip((INITIAL, MEDIAL, FINAL), chars))
    init_idx, med_idx, final_idx = idx
    # final index must be shifted once as
    # final index with 0 points to syllables without final
    final_idx = 0 if final_idx is None else final_idx + 1
    return unichr(0xac00 + 28 * 21 * init_idx + 28 * med_idx + final_idx)
